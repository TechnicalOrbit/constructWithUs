...
if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true) {
  if ($_SERVER['REQUEST_METHOD'] == "POST") {
    $aggerateprice = $_POST['aggerate'];
    $csql = "UPDATE `price` SET `aggerate` = '$aggerateprice' WHERE `price`.`pId` = 1";
  ...
  <div class="d-flex gap-3">
    ...
    <div class="container my-4 mb-5">
      <form method="post" action=<?php htmlspecialchars($_SERVER['PHP_SELF']); ?>>
        <div class="mb-3">
          <label for="aggerate" class="form-label">Aggerate Price</label>
          <input type="file" accept="image/*" class="form-control slider" id="aggerate" name="aggerate">
        </div>
