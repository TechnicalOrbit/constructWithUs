...
if (isset($_SESSION['loggedin']) && $_SESSION['loggedin'] == true) {
  if ($_SERVER['REQUEST_METHOD'] == "POST") {
    $sandprice = $_POST['sand'];
    $csql = "UPDATE `price` SET `sand` = '$sandprice' WHERE `price`.`pId` = 1";
  ...
  <div class="d-flex gap-3">
    ...
    <div class="container my-4 mb-5">
      <form method="post" action=<?php htmlspecialchars($_SERVER['PHP_SELF']); ?>>
        <div class="mb-3">
          <label for="sand" class="form-label">Sand Price</label>
          <input type="file" accept="image/*" class="form-control slider" id="sand" name="sand">
        </div>
